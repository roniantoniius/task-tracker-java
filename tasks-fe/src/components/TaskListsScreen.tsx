...
const TaskListScreen: React.FC = () => {
  ...
  // Fetch task lists saat komponen di-mount
  useEffect(() => {
    ...
    <div className="p-4 max-w-sm w-full">
      ...
      {state.taskLists.map((list) => (
        <Card
          ...
          role="button"
          aria-label={`Select task list: ${list.nama}`}
        >
          <CardBody>
            ...
            <p className="text-sm text-gray-500 mt-2">{list.hitung} tasks</p>
            <Progress
              value={list.progress ? list.progress * 100 : 0}
              className="mt-2"
              color="primary"
              aria-label={`Progress for ${list.nama}: ${list.progress ? list.progress : 0}%`}
            />
          </CardBody>
        </Card>
      ))}
